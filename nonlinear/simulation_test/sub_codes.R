...
# 1. change get_os part
# 2. change plotpred for un-orderd indicator information
##### update 07/26/2017
# 1. modify data2fdr  for RKHS basis
#    - Only for balanced case, dimx=1
# 2. add simpson's rule for integation in computing innerproducts
# 3. add modified gramt function
####
# Some functions from C++. The funciton list is below
##################
# Function with C list
#===============================================================#
# 1. bDiag(A,d) : blog diagonal matrix --
#---------------------------------------------------------------#
...
#===============================================================#
# 3. gam(normX) estimating width-parameter of kernel function -----
#---------------------------------------------------------------#
# input : normX : n x n matrix such that 
#       (normX)_ij = ||x_i - x_j||^2
# value : estimated gamma 
#===============================================================#
# 4. mppower(x,a) : power of matrix (moore penrose type)------
#---------------------------------------------------------------#
# input : x : matrix
# value : x^a ignoring the eigenvectors with eigenvalue < 10^(-15)
#===============================================================#
# 5. matpower2(x,a,epsilon) : power of matrix(regularized moore-penrose)------
#---------------------------------------------------------------#
...
library(fda)
#setwd(readLines("~/pathdir.txt"))
Rcpp::sourceCpp('bspline.cpp')
Rcpp::sourceCpp('sub_codes.cpp')
Rcpp::sourceCpp('nonlinear_subcodes.cpp')
###############################################################
...
simpson=function(ns){
  ...
  wei[2*(1:((ns-1)/2))] = 4
  wei[2*(1:((ns-1)/2-1))+1] = 2
  h=1/(ns-1); wei=wei*(h/3)
  return(wei)
}
## test run ##
# ns=11;tt=seq(0,1,length=ns);qua=function(t) return(sin(t));
# f=numeric();for(i in 1:ns) f=c(f,qua(tt[i]))
# f%*%simpson(ns)
############################################################### 
...
############################################################### 
gramt=function(tte,tto,kern, gamma=NULL){
...
# 2. <x_i, x_j>
# 3. normX = \|X_i - X_j \|^2
data2fdr <- function(x=NULL, tx=NULL, xftn=FALSE,  xtequal=FALSE, gcvall=FALSE, lambdascale=2, HTbasis="gauss",
  ...
  if(xftn){  # x : list  tx : list
    ...
    if(ftnstruct=="rkhs"){
      KT <- gramt(xt.unique, xt.unique, kern=HTbasis) # gram matrix for T
      
      ns = ntx 
      if(ntx %% 2 == 0) ns = ntx + 1
      eval.t <- seq(min(xt.unique), max(xt.unique), length=ns)
      kk <- gramt(xt.unique, eval.t, kern=HTbasis)
      
      if(xtequal){
        x <- makematrix(x, ntx)   # n X nt matrix
        ets <- c(0,exp(seq(log(10^(-5)),log(10),len=ncv1-1)))
        cvresult <- gcv_tune(x, ets, KT)
        et = ets[which.min(cvresult)]
        KT.inv <- matpower(KT, -1, et)
        xcoef <- KT.inv %*% t(x)  # nt X n matrix
        xip <- t(xcoef) %*% KT.inv %*% t(kk) %*% diag(simpson(ns)) %*% kk %*% KT.inv %*% xcoef
      }
      if(!xtequal){ # unbalanced case
        stop("need coding (!xtequal)")
      }
      if(dimx>1){
        stop("need coding (dimx>1)")
      }
      basis <- NULL ; 
      binnerprod=KT;lambdaout=NULL
      xip <- t(xcoef) %*% binnerprod %*% xcoef
    }else if(ftnstruct=="fda"){
      ...
      if(!pca.basis){  # use the basis 
        ...
        lambda_all<- exp(seq(log(10^(-10)),log(10^(-lambdascale)),length=ncv1))
        if(dimx==1){
      ...
      }else if(pca.basis){ # use PCA basis
        nbasis <- 50
        basis <- eval(as.name(paste("create.",basisname,".basis",sep='')))(c(tmin,tmax), nbasis=nbasis) # maxnbasis
        RR<-eval(as.name(paste(basisname,"pen",sep='')))(basis,2)
  ...
  return(list(n=n,ntx=ntx,xcoef=xcoef, xip=xip,  normX=normX, basis=basis, binnerprod=binnerprod,lambdaout=lambdaout, fpca=fpca, et=et))
}

##############################################################
#      gcv tune T (for brownian RKHS H_T)------ 
# x : n X nt matrix
##############################################################
gcv_tune <- function(x, ets, KT){
  n = nrow(x) ; nt = ncol(x)
  ncv = length(ets)
  cvs = rep(0, ncv)
  for(k in 1:ncv){
    ...
    KT.inv = matpower(KT, -1, et)
    H = KT %*% KT.inv
    div = (1 - sum(diag(H)) / nt)^2
    A = t(x) - H %*% t(x)
    cvs[k] = sum(apply(A, 2, function(x) sum(x^2)) ) / div
  }
  return(cvs)
}
...
matern.ftn <- function(J,N, nu, ssq=1){   # J : time, N : number of curves
  ...
  cov.mat.sq <- mppower(cov.mat,1/2)
...
#######################################################################
#            function: projection coefficient of v1 on to v2 
#                        or cos(v1, v2) 
#######################################################################
proj = function(v1,v2){
  return(v1%*%v2/c(sqrt(v2%*%v2)))}
#######################################################################
#           function: spin y vs pred  
#                     slow, how slow you spin, larger number 
#                     means slow; slow =  2000 works well
#                     round: how many cycles
#######################################################################
spin = function(y,pred,slow,round){
  lim = max(abs(pred))
  n = length(y)
  for(theta in seq(0,2*3.1416*round,length=round*slow)){
    xproj = rep(0,n)
    v2 = c(cos(theta),sin(theta))
    for(i in 1:n){
      xproj[i] = proj(pred[i,],v2)}
    plot(xproj,y,xlim=c(-lim,lim),
         yaxt='n',xaxt='n',xlab=" ",ylab=" ")
    lines(c(0,0),c(min(y),max(y)))
  }
}

scatter.3d = function(ppp,y){
  require(plotly)
  # make dataframe of x
  xdata=data.frame(x=ppp[,1], y=ppp[,2], z=ppp[,3],cls = factor(y))
  p <- plot_ly(xdata, x = ~x, y = ~y, z = ~z, color =~cls, colors = c('#BF382A', '#0C4B8E')) %>%
    add_markers() %>%
    layout(scene = list(xaxis = list(title = '1st predictor',
                                     range=c(-2,2)),
                        yaxis = list(title = '2nd predictor',
                                     range=c(-2,2)),
                        zaxis = list(title = '3rd predictor',
                                     range=c(-2,2))))
  return(p)
}
#########################################
